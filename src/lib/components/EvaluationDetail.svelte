...
<dl class="flex justify-start pb-1">
	...
	{#if tool.evaluators && tool.evaluators.length > 0}
		...
		.map(evaluator => evaluator.contact
...
<dl class="flex justify-start pb-1">
	{#if tool.history}
		<dt><a href={tool.history} target="_blank" class="text-blue-600 hover:underline">Click here for history</a></dt>
	{:else}
		<dt>History:</dt>
...
{#each checklistSections as section}
	<div class="mt-6 py-3">
		<h3 class="text-lg lg:text-xl xl:text-2xl">{section.charAt(0).toUpperCase() + section.slice(1)}</h3>
